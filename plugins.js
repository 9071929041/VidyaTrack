(function () {
  ...
  async function renderDailyCollectionWidget() {
    if (document.getElementById("vtDailyTracker")) return;
    ...
    if (window.sbClient) {
      try {
        ...
        todayTotal = (fees || []).reduce((sum, f) => sum + parseFloat(f.amount_paid || 0), 0);
      } catch (e) {
        console.warn("Collection query error", e);
      }
    }
    ...
    tracker.innerHTML = `
      ...
      <button onclick="window.quickDefaulterNotice()" class="bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] px-2.5 py-1 rounded-lg font-bold shadow-xs">
        📢 Fee Notice
      </button>
    `;
  ...
  window.quickDefaulterNotice = async () => {
    if (!window.sbClient) return alert("Database connection loading...");
  ...
  // Re-inject on navigation or tab changes
  const observer = new MutationObserver(() => renderDailyCollectionWidget());
  const app = document.getElementById("app");
  if (app) observer.observe(app, { childList: true, subtree: true });

  setInterval(renderDailyCollectionWidget, 500);
})();
